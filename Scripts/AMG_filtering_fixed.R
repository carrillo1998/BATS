...
combined_quality <- read.delim("~/Downloads/combined_quality.tsv")

viral_checkV <- combined_quality %>%
  filter(checkv_quality %in% c("Complete", "High-quality", "Medium-quality"))

viral_checkV_text_edit <- viral_checkV
viral_checkV_text_edit <- viral_checkV_text_edit %>%
  mutate(contig_id = str_replace(contig_id, "^[^_]+_[^_]+_", ""))
...
conservative_AMGs_final_flag_check_filter2 <- conservative_AMGs_final_flag_check_filter
conservative_AMGs_final_flag_check_filter2$scaffold <- gsub("-cat.*$", "", gsub("__", "_", conservative_AMGs_final_flag_check_filter2$X))
conservative_AMGs_final_flag_check_filter2$scaffold <- gsub("_full", "||full", conservative_AMGs_final_flag_check_filter2$scaffold)
conservative_AMGs_final_flag_check_filter2$scaffold <- gsub("_0_partial", "||0_partial", conservative_AMGs_final_flag_check_filter2$scaffold)
conservative_AMGs_final_flag_check_filter <- conservative_AMGs_final_flag_check_filter2
...
counts <- counts %>%
  mutate(
    diel = rowSums(select(., starts_with("vOTUs_Arche")), na.rm = TRUE)
  ) %>%
  select(-starts_with("vOTUs_Arche"))

colnames(counts) <- c("taxonomy_g", "ko_id", "vOTUs_non_diel", "vOTUs_diel")
...
counts <- counts %>%
  mutate(
    Group = case_when(
      vOTUs_diel > 0 & vOTUs_non_diel == 0 ~ "Unique_diel",
      vOTUs_diel == 0 & vOTUs_non_diel > 0 ~ "Unique_non_diel",
      vOTUs_diel > 0 & vOTUs_non_diel > 0  ~ "Shared",
      TRUE ~ NA_character_
    )
  )
...
final_df <- counts %>%
  ...
  select(host, vOTUs_non_diel, vOTUs_diel, AMG, Group)

write.csv(final_df, file = "AMG_diel_nondiel_host.csv", row.names = FALSE)
...
######### Making supplemental figures 7 and 8 ###########



conservative_AMGs_final_flag_check_filter
diel <- surface_signif_diel
diel$vOTU_names <- gsub("__", "_", diel$vOTU_names)

diel_annotations <- conservative_AMGs_final_flag_check_filter %>%
  filter(scaffold %in% diel$vOTU_names)

non_diel_annotations <- conservative_AMGs_final_flag_check_filter %>%
  filter(!(scaffold %in% diel$vOTU_names))

write.csv(diel_annotations, file = "Supp_table7.csv", row.names = FALSE)
write.csv(non_diel_annotations, file = "Supp_table8.csv", row.names = FALSE)
